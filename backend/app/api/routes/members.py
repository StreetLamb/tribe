...
from app.models import Member, MemberCreate, MemberOut, MemberUpdate, MembersOut, Message, Team
...
def read_members(
    ...
    if current_user.is_superuser:
        ...
        statement = select(Member).where(Member.belongs_to == team_id).offset(skip).limit(limit)
...
def read_member(session: SessionDep, current_user: CurrentUser, team_id: int, id: int) -> Any:
    ...
    """
    if current_user.is_superuser:
      statement = select(Member).join(Team).where(Member.id == id, Member.belongs_to == team_id)
      member = session.exec(statement).first()
    else:
      statement = select(Member).join(Team).where(Member.id == id,  Member.belongs_to == team_id, Team.owner_id == current_user.id,)
      member = session.exec(statement).first()
...
def create_member(
    *, session: SessionDep, current_user: CurrentUser, team_id: int, member_in: MemberCreate
) -> Any:
    """
    Create new member.
    """
